...
def record_operation_end(operation_id):
    ...
    cursor.execute(
        "UPDATE user_operation_logs SET end_time = ?, duration = strftime('%s', ?) - strftime('%s', start_time) WHERE id = ?",
...
def user_center():
    ...
    if st.session_state.logged_in:
        if st.session_state.username != "游客":
            ...
            # 添加导出操作记录为Excel按钮
            if st.button("导出操作记录"):
                excel_data = export_operation_logs_to_excel(st.session_state.username,
                                                            st.session_state.current_login_time)
                st.download_button(
                    label="下载Excel文件",
                    data=excel_data,
                    file_name="操作记录.xlsx",
...
def chapter1():
    ...
    with st.expander("条件概率和独立性判断"):
        ...
        if st.button("计算条件概率", key="cond_prob"):
            ...
            st.write(
                f"根据独立性定义，如果P(A∩B) ≈ P(A)×P(B)，则事件A和B独立。本实验中{'满足' if is_independent else '不满足'}这一条件。")
...
def chapter2():
    ...
    with st.expander("两点分布", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "两点分布是一种最简单的离散概率分布，随机变量只有两个可能的取值：0和1。本实验通过设置成功概率p，观察两点分布的概率分布情况。")
    ...
    with st.expander("二项分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "二项分布描述了n次独立重复伯努利试验中成功次数的概率分布。本实验通过设置试验次数n和成功概率p，观察二项分布的累积分布函数。")
        ...
        if st.button("显示二项分布", key="binomial"):
            ...
            st.write(
                "累积分布函数(CDF)表示成功次数不超过k的概率。从图中可以看出，随着成功次数k的增加，累积概率逐渐增加到1。")
    ...
    with st.expander("泊松分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "泊松分布常用于描述单位时间或空间内随机事件发生次数的概率分布。本实验通过设置参数λ，观察泊松分布的概率质量函数。")
    ...
    with st.expander("正态分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "正态分布是最常见的连续型概率分布，具有对称的钟形曲线。本实验通过调整均值μ和标准差σ，观察正态分布的概率密度函数变化。")
        ...
        if st.button("显示正态分布", key="normal"):
            ...
            st.subheader("实验分析")
            st.write(
                f"正态分布N(μ={mean}, σ={std})呈现对称的钟形曲线，均值μ决定了曲线的中心位置，标准差σ决定了曲线的分散程度。")
    ...
    with st.expander("均匀分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "均匀分布是一种简单的连续型概率分布，在指定区间内各点的概率密度相等。本实验通过设置区间范围，观察均匀分布的概率密度函数。")
        ...
        if st.button("显示均匀分布", key="uniform"):
            ...
            st.subheader("实验分析")
            st.write(
                f"均匀分布U({min_val}, {max_val})在区间[{min_val}, {max_val}]内的概率密度为常数1/({max_val}-{min_val})，区间外的概率密度为0。")
    ...
    with st.expander("指数分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "指数分布常用于描述两个连续事件发生的时间间隔的概率分布。本实验通过设置率参数λ，观察指数分布的概率密度函数。")
    ...
    with st.expander("泊松分布近似二项分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "当二项分布的试验次数n很大而成功概率p很小时，二项分布B(n,p)可以用泊松分布P(λ=np)近似。本实验通过对比两种分布，验证这一近似效果。")
    ...
    with st.expander("二项分布近似正态分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "当二项分布的试验次数n足够大，且np和n(1-p)都较大时，二项分布B(n,p)可以用正态分布N(np, np(1-p))近似。本实验通过对比两种分布，验证这一近似效果。")
...
def chapter3():
    ...
    with st.expander("二维离散型随机变量", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "对于二维离散型随机变量(X,Y)，本实验通过设置联合概率分布，计算并可视化展示X和Y的边缘分布，理解联合分布与边缘分布的关系。")
    ...
    with st.expander("二维连续型随机变量"):
        ...
        st.subheader("实验内容")
        st.write(
            "对于二维连续型随机变量(X,Y)，本实验通过生成服从特定分布的样本数据，绘制散点图展示联合分布，并通过直方图展示X和Y的边缘分布。")
...
def chapter4():
    ...
    with st.expander("数字特征计算", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "对于选定的概率分布，生成样本数据，计算样本的数字特征（均值、方差、标准差、偏度、峰度），并与理论值进行比较，验证样本估计的准确性。")
...
def chapter5():
    ...
    with st.expander("中心极限定理", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "从选定的总体分布中重复抽取一定大小的样本，计算每个样本的均值，观察这些样本均值的分布形态，验证随着样本量增大，样本均值分布是否趋近于正态分布。")
    ...
    with st.expander("大数定律"):
        ...
        st.subheader("实验内容")
        st.write(
            "从选定的总体分布中抽取样本，随着样本量逐渐增加，计算累积样本均值，观察样本均值是否会逐渐收敛于总体的理论期望。")
...
def chapter6():
    ...
    with st.expander("次序统计量的分布"):
        ...
        st.subheader("实验内容")
        st.write(
            "从选定的总体分布中重复抽取样本，计算每个样本的最小值、最大值和中位数等次序统计量，观察这些统计量的分布形态。")
...
def chapter7():
    ...
    with st.expander("矩估计", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "从均匀分布总体中抽取样本，利用矩估计方法估计总体参数（下限a和上限b），比较估计值与真实值的差异，观察样本量对估计精度的影响。")
    ...
    with st.expander("最大似然估计"):
        ...
        st.subheader("实验内容")
        st.write(
            "从正态分布总体中抽取样本，利用最大似然估计方法估计总体均值μ和标准差σ，比较估计值与真实值的差异，观察样本量对估计精度的影响。")
        ...
        if st.button("执行最大似然估计", key="mle_est"):
            ...
            st.write(
                f"本实验中，均值μ的估计误差为{abs(mu_hat - mu_true):.4f}，标准差σ的MLE估计误差为{abs(sigma_hat_mle - sigma_true):.4f}。")
            st.write(
                "对于正态分布，均值的最大似然估计与矩估计相同，但标准差的最大似然估计是有偏的（偏小），样本标准差是其无偏估计。")
    ...
    with st.expander("估计量的有效性"):
        ...
        st.subheader("实验内容")
        st.write(
            "从选定的总体分布中重复抽取样本，计算参数的估计量，估计该估计量的方差，并与Cramér-Rao下界比较，判断该估计量是否为有效估计量。")
        ...
        if st.button("评估估计量有效性", key="eval_eff"):
            ...
            st.write(
                f"样本均值估计量的实际方差为{var_estimate:.6f}，{'' if var_estimate < avg_crlb * 1.05 else '未'}达到理论下限。")
...
def chapter8():
    ...
    with st.expander("置信区间估计", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "从正态总体中抽取样本，在给定的置信水平下，计算总体均值的置信区间，观察置信区间是否包含总体真实均值，理解置信区间的统计含义。")
        ...
        if st.button("计算置信区间", key="calc_ci"):
            ...
            ax.axvline(x=ci_lower, color='green', linestyle='--',
                       label=f"{confidence_level * 100}% CI: [{ci_lower:.2f}, {ci_upper:.2f}]")
            ...
            st.write(
                f"{confidence_level * 100}%置信区间表示，在多次重复抽样中，大约有{confidence_level * 100}%的置信区间会包含总体真实均值。")
            st.write(
                f"本次计算的置信区间为[{ci_lower:.4f}, {ci_upper:.4f}]，{'包含' if ci_lower <= population_mean <= ci_upper else '不包含'}总体真实均值。")
    ...
    with st.expander("正态总体参数的置信区间"):
        ...
        if st.button("生成置信区间", key="norm_ci"):
            ...
            st.write(
                f"本次实验中，均值的{confidence_level * 100}%置信区间{'包含' if ci_mean[0] <= pop_mean <= ci_mean[1] else '不包含'}真实均值，")
            st.write(
                f"方差的{confidence_level * 100}%置信区间{'包含' if ci_var[0] <= pop_std ** 2 <= ci_var[1] else '不包含'}真实方差。")
...
def chapter9():
    ...
    with st.expander("单样本t检验", expanded=True):
        ...
        st.subheader("实验内容")
        st.write(
            "从正态总体中抽取样本，设定原假设H₀: μ=μ₀和备择假设H₁: μ≠μ₀，计算t统计量和P值，根据显著性水平做出是否拒绝原假设的决策。")
        ...
        if st.button("执行单样本t检验        ", key="one_sample_t_test"):
            ...
            st.write(
                f"3. 真实均值为{true_mean}，{'与假设值存在差异' if true_mean != hypothesized_mean else '与假设值一致'}，检验结果{'正确' if (decision == '拒绝原假设' and true_mean != hypothesized_mean) or (decision == '不拒绝原假设' and true_mean == hypothesized_mean) else '错误'}。")
    ...
    with st.expander("两样本t检验"):
        ...
        st.subheader("实验内容")
        st.write(
            "从两个正态总体中分别抽取样本，设定原假设H₀: μ₁=μ₂和备择假设H₁: μ₁≠μ₂，通过t检验判断两个总体的均值是否存在显著差异。")
        ...
        if st.button("执行两样本t检验", key="two_sample_t_test"):
            ...
            st.write(
                f"1. 本次检验中，总体1均值为{mean1}，总体2均值为{mean2}，{'存在真实差异' if mean1 != mean2 else '不存在真实差异'}。")
            st.write(
                f"2. 检验结果{'正确' if (decision == '拒绝原假设' and mean1 != mean2) or (decision == '不拒绝原假设' and mean1 == mean2) else '错误'}。")
            st.write(
                f"3. 当两总体方差{'' if equal_var else '不'}相等时，使用{'合并' if equal_var else '分开'}的方差估计。")
